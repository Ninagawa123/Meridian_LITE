// Meridan Board -LITE-
// ESP32用スケッチ　20220703版
#define VERSION "Hello, This is Meridian_LITE_20220703." // バージョン表示
...
// 
// 上記を搭載している場合、
// サーボについてはvoid setup()内の idl_mt[x], idr_mt[x] でIDごとのサーボ搭載ありなしを設定。デフォルトはKHR-3HVの22軸設定。
...
// 20220703 配列のリモコン番地を[80]~[83]から[15]〜[18]に移動。
// 20220703 モーション用の時間設定を[01]から[19]に移動。
// 20220703 配列のフレーム同期カウンタを[01]に移動。
// 20220703 配列[00]番の特殊コマンドを1000番台→10000番台に１桁移動。
// 20220706 説明文の誤植などを微修正。
...
#define BT_MAC_ADDR "xx:xx:xx:xx:xx:xx" // ESP32自身のBluetoothMACアドレス（本プログラムを実行しシリアルモニタで確認したものを書き込む）
  ...
  { // KRR5FH(KRC-5FH)をICS_R系に接続している場合
    ...
    if (joypad_frame_count >= JOYPAD_POLLING)
    {
      ...
      if (buttonData != KRR_BUTTON_FALSE) // ボタンデータが受信できていたら
      {
        ...
        if ((button_1 & 368) == 368)
          pad_btn += 8; //右側十字ボタン全部押しなら select押下とみなす
        else
        {
          // 右側十字ボタン
