...
#define UDP_SEND_PORT 22232 // 送り先のポート番号
#define UDP_RECV_PORT 22234 // このESP32のポート番号

// Wifi用のESP32固定IPアドレスの設定
// ※config.hの MODE_FIXED_IP を1に設定することで有効
#define FIXED_IP_ADDR    "192.168.xxx.xxx" // ESP32の固定IP
#define FIXED_IP_GATEWAY "192.168.xxx.xxx"   // ゲートウェイ(ルーターのIPアドレス)
#define FIXED_IP_SUBNET  "255.255.255.0" // サブネットマスク

// Ethernet用のESP32固定IPアドレスの設定
// ※config.hの MODE_ETHER を1に設定に設定することで有効
#define ETHER_LOCAL_IP "192.168.xxx.xxx"      // 有線LAN使用時のESP32のIPアドレス
#define ETHER_GATEWAY  "192.168.xxx.xxx"      // 有線LAN使用時のPCのIPアドレス
