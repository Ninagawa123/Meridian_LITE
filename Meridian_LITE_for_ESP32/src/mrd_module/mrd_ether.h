...
const char *normalizeMacString(const char *macStr) {
  static char normalized[24];
  int idx = 0;
  for (int i = 0; macStr[i] != '\0' && idx < (int)sizeof(normalized) - 1; i++) {
    if (macStr[i] != ' ' && macStr[i] != '\t') {
      normalized[idx++] = macStr[i];
    }
  }
  normalized[idx] = '\0';
  return normalized;
...
bool parseMacAddress(const char *macStr, byte *macBytes) {
  ...
  // 余分なスペースを除去して正規化
  const char *cleanMac = normalizeMacString(macStr);

  // 正規化後の長さチェック（17文字必要："XX:XX:XX:XX:XX:XX"）
  if (strlen(cleanMac) != 17) {
    return false;
  }
