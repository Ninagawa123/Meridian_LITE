// mrd_servo.cpp - Servo distributor implementation
...
bool mrd_servo_begin(UartLine a_line, int a_servo_type) {
  ...
  case 1:
    ...
    return false;
  case 11:
    // I2C_SERVO_TYPE_PCA9685 to PWM [WIP]
    return false;
  case 21:
    // RSxTTL (FUTABA) [WIP]
    return false;
...
bool mrd_servo_all_off(Meridim90Union &a_meridim, HardwareSerial &a_serial) {
  for (int i = 0; i < MRD_SV_SLOTS; i++) {
    a_meridim.sval[i * 2 + MRD_L_ORIGIDX] = 0; // L系統サーボのコマンドをオフ
    a_meridim.sval[i * 2 + MRD_R_ORIGIDX] = 0; // R系統サーボのコマンドをオフ
  }
  a_serial.println("All servos torque off.");
  return true;
}
...
uint8_t mrd_servos_make_errcode_lite(const ServoParam &a_sv) {
