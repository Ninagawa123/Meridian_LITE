...
public:
  ...
  /// @brief ESP32本体のMACアドレス(WiFi, Bluetooth)をシリアルモニタに出力する.
  void esp_mac_address() {
    // WiFi MAC address
    m_serial.print("WiFi MAC   ");
    m_serial.println(WiFi.macAddress());
    ...
    m_serial.print("BT MAC     ");
    m_serial.println(bt_mac_str);
  }
  ...
  bool servo_protocol(UartLine a_line, int a_servo_type) {
    if (a_servo_type > 0) {
      ...
      case 1:
        ...
        break;
      case 11:
        m_serial.print("I2C_SERVO_TYPE_PCA9685 to PWM");
        m_serial.println(" - Not supported yet.");
        break;
      case 21:
  ...
  void esp_ip(bool a_flg_fixed_ip) {
    m_serial.println("WiFi successfully connected.");
    m_serial.println("PC's IP address target => " + String(WIFI_SEND_IP));
    m_serial.print("ESP32's IP address => ");
    m_serial.print(WiFi.localIP().toString());
    m_serial.println(a_flg_fixed_ip ? " (*Fixed)" : "");
  }
