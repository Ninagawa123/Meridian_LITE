// sv_ics.cpp - KONDO ICS servo implementation
...
float mrd_sv_process_ics(int a_servo_id, int a_cmd, float a_tgt, float a_tgt_past, float a_trim,
  ...
  if (val_tmp == -1) { // サーボからの返信信号を受け取れなかった場合
    ...
    a_err_cnt++;
    if (a_err_cnt >= SV_LOST_ERR_WAIT) { // 一定以上の連続エラーで通信不能とみなす
      a_err_cnt = SV_LOST_ERR_WAIT;
      a_stat = 1;
    }
  } else {
    a_err_cnt = 0;
...
void mrd_sv_drive_ics_double(Meridim90Union &a_meridim, ServoParam &a_sv) {
  for (int i = 0; i < a_sv.num_max; i++) {
    ...
    if (a_sv.ixl_mount[i]) { // 43は近藤科学のICSサーボ
      a_sv.ixl_tgt[i] = mrd_sv_process_ics(
    ...
    if (a_sv.ixr_mount[i]) { // 43は近藤科学のICSサーボ
      a_sv.ixr_tgt[i] = mrd_sv_process_ics(
