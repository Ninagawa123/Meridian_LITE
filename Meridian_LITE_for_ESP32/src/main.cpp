...
void setup() {
  ...
  timer = timerBegin(0, 80, true);                               // タイマーの設定(timer0, 分周比80=1μs分解能)
...
void loop() {
  ...
  // pad_arrayに、meridimで受けたパッドの値を代入する(ボタン+スティック+L2R2の全4ワード)
  memcpy(pad_array.sval, &s_udp_meridim.sval[MRD_PAD_BUTTONS], PAD_LEN * sizeof(short));
